...
const Profile = () => {
  ...
  const [travel, setTravel] = useState(0);         // 1 = Yes, 0 = No
  const [visibility, setVisibility] = useState(0); // 0 = Normal, 1 = Private, 2 = Pause/In Tour
  const [notification, setNotification] = useState(0); // 0 = Email, 1 = Email + SMS
  ...
  const [languageOptions, setLanguageOptions] = useState([]); // { id, name }
  ...
  const fields = [
    ...
    { label: "Dress Size *", name: "dressSize",type: "text" },
  ...
  const toggleOptionAvailableFor = (item) => {
    ...
    console.log(selectedAvailableForIds)
  };
